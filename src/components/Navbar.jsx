...
const Navbar = () => {
  ...
  return (
    <header className="bg-white fixed top-0 left-0 w-full z-50 border-b border-red-600 backdrop-blur-md">
      <div className="max-w-screen-2xl mx-auto flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center flex-wrap justify-end gap-x-4 sm:gap-x-6 md:gap-x-8 lg:gap-x-10 flex-grow">
          ...
          <form
            action="/search"
            method="get"
            className="hidden sm:block relative w-full max-w-[160px] sm:max-w-[200px] md:max-w-[220px] lg:max-w-[250px]"
          >
            <input
              type="text"
              name="q"
            ...
            {navLinks.map(link => (
              <Link
                ...
                className={`relative underline-animation ${
                  active === link.href ? 'active-link' : 'hover:text-blue-600 text-gray-800'
                }`}
              >
                {link.label}
              </Link>
